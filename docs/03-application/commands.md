...
| 規則 | 說明 |
| --- | --- |
| 動詞開頭 | `SubmitLeaveRequest`, `ApproveLeaveRequest`, `RunPayroll` |
...
| Approval | `ApproveLeaveRequest`, `ApproveOvertimeRequest` | 必驗證 approver capability |
| HR / Payroll | `RunPayroll`, `PublishSalarySlips` | 敏感寫入，server-only |
| Security | `GrantCapability`, `ExportAuditLogs` | 必有 audit 與最小權限 |
