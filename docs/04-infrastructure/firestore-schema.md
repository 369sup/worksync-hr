...
| `leave_balance_ledgers` | Leave | `employee_id`, `leave_type_id`, `entries`, `version` | `adjustment_reason` |
...
| `payroll_periods` | Payroll | `period`, `status`, `payroll_input_version` | `gross_total`, `net_total`, `banking_batch_ref` |
...
- Collection owner 只表示 persistence ownership；跨 Context 讀取仍必須透過 mapper 與 Published Language port。
